...
class SparseRegressor(BaseEstimator, ClassifierMixin, TransformerMixin):
    ...
    def fit(self, X, y):
        self.threshold_ = 1e-4
        self.n_nonzero_coefs_ = int(y.sum(axis=1).max())
        return self
    ...
    def _run_model(self, model, L, X):
        ...
        for idx in range(len(X)):
            x = X.iloc[idx].values
            model.set_params(n_nonzero_coefs=self.n_nonzero_coefs_)
            model.fit(L, x)
...
class CustomSparseEstimator(BaseEstimator, RegressorMixin):
    def __init__(self, n_nonzero_coefs=1):
...
def get_estimator():
    ...
    sr = SparseRegressor(Ls, parcel_indices, custom_model)
    return sr
